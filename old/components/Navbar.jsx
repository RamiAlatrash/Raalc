        ...
        <div className="flex items-center justify-between h-full">
          ...
          >
            ...
            >
              ...
              <span className={`text-sm font-bold -mt-2 ${lang === 'ar' ? 'font-arabic' : ''}`}>{navText[lang].slogan}</span>
            </span>
          </button>
          ...
          <div className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className="px-8 py-4 rounded-md text-xl font-semibold text-brand-dark-grey hover:text-primary transition duration-200 transform hover:scale-110"
              >
                {item.name}
              </button>
            ))}
