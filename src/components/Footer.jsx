...
const Footer = ({ scrollToSection }) => {
  ...
  return (
    <footer className="relative bg-gradient-to-br from-[#e3ecf7] via-[#f5f7fa] to-[#d1e3f8] border-t border-border text-foreground overflow-hidden">
      <img
        src="/images/robot-images/robot15.png"
        alt=""
        className="absolute -bottom-4 -left-8 w-24 opacity-30 pointer-events-none select-none ltr-preserve"
        aria-hidden="true"
      />
      {/* Animated Blob */}
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
          <div className="space-y-4">
            ...
            <p className={`text-base md:text-lg text-brand-dark-grey ${lang === 'ar' ? 'font-arabic rtl-text' : 'font-body'}`}>
              {t('description_p1')}
            </p>
            <p className={`text-base md:text-lg text-brand-dark-grey ${lang === 'ar' ? 'font-arabic rtl-text' : 'font-body'}`}>
              {t('description_p2')}
          ...
          <div className="flex flex-row items-start space-x-4 relative">
            <div className="flex flex-col justify-start">
              <p className={`font-heading text-xl text-primary mb-6 ${lang === 'ar' ? 'font-arabic rtl-text' : ''}`}>{t('quick_links')}</p>
              <ul className="space-y-4">
                {quickNavLinks.map((link) => (
                  <li key={link.name}>
                    <button 
                      onClick={() => scrollToSection(link.id)} 
                      className={`hover:text-primary transition-colors duration-300 text-base md:text-lg text-brand-dark-grey focus:outline-none ${lang === 'ar' ? 'font-arabic rtl-text' : 'font-body'}`}
                    >
                      {link.name}
                    </button>
                  </li>
            ...
            <img
              ...
              alt="Footer Robot"
              className="w-24 h-24 md:w-36 md-h-36 ml-4 animate-footer-robot-move ltr-preserve"
              style={{ filter: 'drop-shadow(0 8px 24px rgba(0,0,0,0.10))' }}
            />
          </div>
        </div>

        <div className={`border-t border-border pt-8 mt-8 text-center text-sm text-brand-dark-grey ${lang === 'ar' ? 'font-arabic rtl-text' : 'font-body'}`}>
